...
SRC_PATH = 'C:\\projects\\detectron2\\datasets\\lesion\\data\\'
...
def fix_save_images(src_path, out_path):
	...
	print("Finding bounding box coords...")
	for im in images:
		## move
		...
		if name.index.size > 0:
			...
			dims = coords_df["Image_size"][name.index[0]].split(',')

			if "train" in im:
				coco_image = CocoImage(im, height=dims[1], width=dims[0])
